...
main(int argc, char *argv[])
{
	...
	try {
		...
		// The old metadata gets replaced but than deleted.
		// Atomically this means deleting the old metadata
		ssrc->publish(pr2);
		...
		if (cntRi(pres, ip, mac, DELETE) != 1)
			cerr << "metadata not found" << endl;

		if (checkMdValue(pres->getDeleteResults().front(), "1"))
			cerr << "wrong metadata found" << endl;
