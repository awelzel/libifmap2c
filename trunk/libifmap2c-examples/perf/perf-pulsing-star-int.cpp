...
static void
checkAddedArmPart(ARC *arc, int step) {
	PollResult *pres = arc->poll();

	if (step != 0) {
		checkRiCnt(pres, DELETE, "grow delete result", armCount);
		checkRiCnt(pres, UPDATE, "grow update result", 2 * armCount);
	} else {
...
deleteArmPart(SSRC *ssrc, int step) {
	...
	forEachArm(i) {
		...
		plist.push_back(Requests::createPublishDelete(FILTER_MATCH_ALL,
					root->clone(), last->clone()));
...
static void
checkDeletedArmPart(ARC *arc, int step) {
	PollResult *pres = arc->poll();
	
	if ((armLength - step - 1) != 0) {
		checkRiCnt(pres, DELETE, "shrink delete result", 2 * armCount);
		checkRiCnt(pres, UPDATE, "shrink update result shrink", armCount);
	} else {
		checkRiCnt(pres, DELETE, "shrink delete result shrink last", armCount);
	}
