...
{
	cerr << "usage: " << prog << " update|delete ar ip"
		INDEPENDENT_USAGE_STRING << endl;
}

int main(int argc, char* argv[])
{
	char *arArg1, *arArg2, *op;
	...
	ar1 = Identifiers::createAr(arArg2);
	ar2 = Identifiers::createAr(arArg1);

	if (isUpdate(op)) {
		role= Metadata::createRole("somerole");
		subReq = Requests::createPublishUpdate(role, ar1, ar2,
				forever);
	} else {
